...
bool D3D11_Pipeline::Begin(const D3D11_PipelineState& pipeline_state)
{
    ...
    // Primitive Topology
    {
        if (bind_primitivetopology_id != pipeline_state.primitive_topology)
        {
            device_context->IASetPrimitiveTopology(pipeline_state.primitive_topology);
            bind_primitivetopology_id = static_cast<uint>(pipeline_state.primitive_topology);
        }
    }

    // Vertex Shader
    {
        if (pipeline_state.vertex_shader != nullptr)
        {
            ...
            if (new_shader != nullptr)
            {
                ...
                device_context->VSGetShader(&current_shader, instance, 0);
    ...
    // Pixel Shader
	{
		...
		device_context->PSGetShader(&current_shader, instance, 0);
    ...
    // Blend State
    {
        if (pipeline_state.blend_state && pipeline_state.blend_state->GetResource())
        {
            if (bind_blend_state_id != pipeline_state.blend_state->GetID())
            {
                ...
                uint samper_mask = pipeline_state.blend_state->GetSampleMask();

                device_context->OMSetBlendState(pipeline_state.blend_state->GetResource(), blend_factor, samper_mask);
...
void D3D11_Pipeline::SetShaderResource(const uint& slot, const uint& scope, const D3D11_Texture* resource)
{
    ...
    ID3D11ShaderResourceView* shader_resource[]{ resource->GetResource(), };

    if (scope & ShaderScope_VS) device_context->VSSetShaderResources(slot, 1, shader_resource);
    if (scope & ShaderScope_PS) device_context->PSSetShaderResources(slot, 1, shader_resource);
}
